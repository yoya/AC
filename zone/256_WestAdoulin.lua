...
M.automatic_routes = {
    ...
    wp_inv = { route="inv" },
    hp_2_M = { route="hp2unity" },
    moghouse = {
	{ route="eminence", contents="Redeem" }, -- エミネンスのガルカ
	{ route="mog2unity", contents="UnityWanted" }, -- 子ミスラ
	{ route="mog2unity", contents="Peculiar" }, -- 子ミスラ
    },
}

local sell_items = {
    -- 4101, -- 水のクリスタル 売らない可能性あるので
    5945, -- プライズパウダー
    12305, -- アイスシールド
    12385, -- アケロンシールド
    12387, -- ケーニヒシールド
...
local sell_item_id_set = utils.table.convert_array_to_set(sell_items)

local selljunk_running = false

function M.tick(player)
    local ac_move = require 'ac/move'
    if acitem.inventory_has_item_in_set(sell_item_id_set) then
