-- ラテーヌ高原

local M = { id = 102 }

M.routes = {
    -- テレポホラ
    shat = {
	{x=420,y=20,z=19.1}, {x=400,y=16}, {a="mount"},
	{x=373.3,y=10.6}, {x=351,y=-9.7}, {x=316.5,y=-58.1},  {a="dismount"},
	{x=336.8,y=-59.7}, {a="f8touch"}
    },
    dim = {
	{x=420,y=20,z=19.1}, {a="mount"},
	{x=373.3,y=10.6}, {x=351,y=-9.7},
	{x=316.5,y=-58.1},{x=318,y=-64.5},
	{x=350,y=-107}, {x=368.8,y=-128},
	{x=417.2,y=-163.3, d=2}, {a="dismount"},
	{x=419.6,y=-143.6}, {a="f8touch"}
    },
    -- ホラリングから
    circle = {  -- カーバンクルのいるところ
	{x=420,y=-150,z=19.5}, {x=416,y=-139},
	{a="mount"}, {x=372,y=-130}, {x=281,3,y=-79.7},
	{x=211.4,y=-46.7}, {x=147.4,y=-23.8},
	{x=102.8,y=-3.9}, {x=42,y=25}, {x=-18.4,y=55.8}, -- {x=-60.4,y=72.4},
	{x=-60.1,y=94.7}, {x=-136.9,y=175.7},
	{x=-175.1,y=245.7}, {a="dismount"},
	--{x=-177.9,y=251}, {a="f8touch"}
	{x=-178,y=252}, {target="???"}, {a="touch"}
    },
    -- 本ワープ
    book2dim = {
	{x=774,y=-19,z=29},{x=768,y=-10,d=2}, {a="mount"},
	{x=762,y=-2,d=2}, {x=747,y=20,d=2},
	{x=709,y=60,d=1},  {x=688,y=69,d=1}, {x=679,y=77},
	{x=641,y=78}, {x=608,y=83}, {x=599,y=82}, {x=586,y=70},
	{x=575,y=60,d=1}, {x=568,y=48,d=1},
	{x=578,y=-57,d=1}, {x=576,y=-65,d=1}, {x=544,y=-94},
	{x=485,y=-121,d=1}, {x=428,y=-163}, {x=421,y=-164},
	{x=420,y=-152}, {a="dismount"},
	{x=420.1,y=-143.2,z=19.1}, {a="f8touch"}
    },
    -- ジュノWPワープ
    frag = { -- 水の祈り (古代石碑巡礼)
	{x=55.6,y=-334.8,z=37.7},{x=33.7,y=-342.2},
	{a="mount"}, {x=11.9,y=-349.5},
	 {x=-23,y=-390}, {x=-66,y=-416},
	{x=-88,y=-438}, {x=-124,y=-445}, {x=-134,y=-458},
	{x=-146.5,y=-494.5}, {a="dismount"},
	{x=-170,y=-506.5},
    },
    -- 指輪ワープ
    ring2dim = {
	{x=420,y=-150,z=19.6}, {x=420,y=-144,z=19.6},
	{target="Dimensional Portal"}, {a="touch"}
    },
    maw = {
        {x=-561.1,y=640.9,z=0,desc="Cavernous Maw(アビセア扉)"}, {wait=2},
        {target="Cavernous Maw"},
        {a="touch"}, {wait=1}, {a="up"}, {a="touch"}
    },
}

M.essential_points = {
    book = {x=774,y=-19,z=29}, -- 本ワープ
    ring = {x=420,y=-150,z=19.6}, -- 指輪ワープ
    from_jueno = {x=-561.1,y=640.9,z=0},
}

M.automatic_routes = {
    book = { route="book2dim", need_level=20 },
    ring = { route="ring2dim" },
    from_jueno = { route="maw" },
}

return M
